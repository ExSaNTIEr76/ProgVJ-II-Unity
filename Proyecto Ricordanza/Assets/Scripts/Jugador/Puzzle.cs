...
public class Puzzle : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.F1) && inventario.ContainsKey("Boxeo"))
        {
            UsarInventario(inventario["Boxeo"]);
        }

        if (Input.GetKeyDown(KeyCode.F2) && inventario.ContainsKey("Dardo")) {
            UsarInventario(inventario["Dardo"]);
        }
        ;
        if (Input.GetKeyDown(KeyCode.F3) && inventario.ContainsKey("Bomba"))
        {
            UsarInventario(inventario["Bomba"]);
        }
    }
